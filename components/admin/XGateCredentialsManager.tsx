...
export default function XGateCredentialsManager() {
  ...
  useEffect(() => {
    fetch('/api/admin/config')
      .then((res) => res.json())
      .then((data) => {
        setEmail(data.XGATE_EMAIL || '')
        setPassword(data.XGATE_PASSWORD || '')
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/admin/config', {
        ...
        body: JSON.stringify({
          XGATE_EMAIL: email.trim(),
          XGATE_PASSWORD: password,
        }),
      })
      if (res.ok) {
        setSaved(true)
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-1">
          Configure o e-mail e a senha da sua conta XGate para gerar PIX na hora da compra. Sem isso, o PIX não será gerado automaticamente.
        </p>
      </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 max-w-xl">
        <p className="text-sm text-amber-800">
          <strong>Webhook:</strong> No painel da XGate, configure a URL de webhook para{' '}
          <code className="bg-amber-100 px-1 rounded">https://seu-dominio.com/api/payments/webhook</code> para confirmação automática do PIX.
        </p>
      </div>

      <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 max-w-xl">
        <p className="text-sm text-gray-700">
          <strong>Segurança:</strong> As credenciais ficam armazenadas no banco (Config) e são usadas apenas no servidor para criar o PIX.
        </p>
      </div>
    </div>
  )
